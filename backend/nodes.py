...
class FuncDefNode:
    def __init__(self, var_name_tok, arg_name_toks, exec_code, auto_ret):
        self.var_name_tok = var_name_tok
        ...
        if self.var_name_tok:
            self.start = self.var_name_tok.start
        elif len(self.arg_name_toks) >= 1:
            self.start = self.arg_name_toks[0].start
        else:
            self.start = self.exec_code.start
...
class CallFuncNode:
    def __init__(self, caller, arg_nodes):
        ...
        if len(self.arg_nodes) >= 1:
            self.end = self.arg_nodes[-1].end
        else:
            self.end = self.caller.end
